...
uint16_t VPositive[40], VNegative[40], PFuse[40], OFuse[40], NFuse[40], CPositive[40], CNegative[40], Temperature[40], Switch[40], Life[40], DOD[40], AH[40], Cycles[40] = {0};
int16_t TemperatureTemp[40], CurrentPosTemp[40], CurrentNegTemp[40];
  ...
  if (client) {
    ...
    while (client.connected()) {
     ...
     Serial.println("TCP connected"); Serial.flush();
      ...
      modbusTCPServer.writeInputRegisters(36, VNegative, 36);    delay (2); 
      modbusTCPServer.writeInputRegisters(72, PFuse, 36);        delay (2);
      modbusTCPServer.writeInputRegisters(108, OFuse, 36);       delay (2);
      modbusTCPServer.writeInputRegisters(144, NFuse, 36);       delay (2);
      modbusTCPServer.writeInputRegisters(180, CPositive, 36);   delay (2);
      modbusTCPServer.writeInputRegisters(216, CNegative, 36);   delay (2);
      modbusTCPServer.writeInputRegisters(252, Temperature, 36); delay (2);
      modbusTCPServer.writeInputRegisters(288, Switch, 36);      delay (2);
      modbusTCPServer.writeInputRegisters(324, Life, 36);        delay (2);
      modbusTCPServer.writeInputRegisters(360, DOD, 36);         delay (2);
      modbusTCPServer.writeInputRegisters(396, Cycles, 36);      delay (2);
      modbusTCPServer.writeInputRegisters(432, AH, 36);          delay (2);
      delay (50);               modbusTCPServer.poll();        delay (100);
      // Serial.println("check point 2"); Serial.flush(); delay (10); /////////////////////////////////////////////////////////////////////
      }
...
void loop() {
  ...
  if (millis() - lastMS >= 100) {
    lastMS = millis();
    for (int i = 0; i < 10; i++)
    {pagePtr = myNex.readNumber("PagePtr.val"); delay (3);}
